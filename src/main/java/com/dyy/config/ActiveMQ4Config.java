 ...
 *
 * @author 段杨宇
 * @create 2017-12-26 17:36
 **/
@EnableJms
@Configuration
public class ActiveMQ4Config {

    private static ActiveMQQueue testQueue = null;

    @Bean(Constant.TEST_QUEUE)
    public ActiveMQQueue getTestQueue(){
        if(testQueue == null){
            testQueue = new ActiveMQQueue(Constant.TEST_QUEUE);
        }
        return testQueue;
    }
